...
# Número de waypoints esperados en el YAML
_N_GOALS = 4
...
class Bug2Node(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('mline_tolerance',    0.10)

        # ── Parámetros de waypoints (del YAML del mundo) ──────────────────────
        ...
        # ── Trayectoria ───────────────────────────────────────────────────────
        self.waypoints      = self._load_waypoints()   # lista de (x, y)
        self.waypoint_index = 0                        # waypoint activo

        if not self.waypoints:
            self.get_logger().error(
                'No se cargaron waypoints. Verifica que el config YAML del '
                'mundo se está pasando en el launch file.'
            )
        else:
        ...
        # Posición de inicio de cada tramo — define la M-line
        self.start_x     = None
        self.start_y     = None
    ...
    def _load_waypoints(self) -> list:
        """
        Lee trajectory_goals.goal_<i>.{x,y} de los parámetros ROS 2.
        Retorna lista ordenada de (x, y). Ignora goals con x==0 y y==0.
        """
        waypoints = []
        for i in range(1, _N_GOALS + 1):
    ...
    def _advance_to_next_waypoint(self):
        """Avanza al siguiente waypoint en ciclo cerrado."""
        prev_idx = self.waypoint_index
        self.waypoint_index = (self.waypoint_index + 1) % len(self.waypoints)

        # La posición actual se convierte en el nuevo start de la M-line
        self.start_x        = self.x
        self.start_y        = self.y
        self.state          = GO_TO_GOAL
        self.hit_dist       = float('inf')
    ...
    def odom_cb(self, msg: Odometry):
        ...
        if not self._odom_ready:
            ...
            self.get_logger().info(
                f'M-line inicial: ({self.start_x:.2f}, {self.start_y:.2f}) '
                f'→ goal_1 ({self.goal_x:.2f}, {self.goal_y:.2f})'
            )
    ...
    def _step_go_to_goal(self, dist: float):
        ...
        if not self._pid_goal_sent:
            ...
            self.get_logger().info(
                f'Setpoint → PID: ({self.goal_x:.2f}, {self.goal_y:.2f})'
            )
    ...
    def _step_wall_follow(self, dist: float):
        ...
        # Salida Bug 2: sobre la M-line Y más cerca del goal que el hit point
        if front_free and on_mline and closer:
            self.state          = GO_TO_GOAL
            self._pid_goal_sent = False
            self.get_logger().info(
                f'M-line alcanzada (d_mline={self._dist_to_mline():.3f} m), '
    ...
    def debug_log(self):
        ...
        self.get_logger().info(
            ...
            f'  Dist a M-line  : {mline:.3f} m  (tol={self.mline_tolerance:.2f} m)\n'
